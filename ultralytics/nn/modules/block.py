...
class C2f_Heat(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""

    def __init__(self, c1: int, c2: int, n: int = 1, drop_path: float = 0.1, res: int = 14, 
        use_checkpoint: bool = False, norm_layer: Callable[..., nn.Module] = LayerNorm2d, 
        post_norm: bool = True, layer_scale=None, mlp_ratio: float = 4.0, 
        infer_mode: bool = False, e: float = 0.5):
        """Initialize a CSP bottleneck with 2 convolutions.
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(HeatBlock(res=res,
                                        hidden_dim=self.c, 
                                        drop_path=drop_path,
                                        ...
                                        infer_mode=infer_mode) for _ in range(n))
...
class eSEModule(nn.Module):
    def __init__(self, c1: int, c2: int):
        ...
        self.fc = nn.Conv2d(c1, c2, kernel_size=1,
                             padding=0)
...
class eSE_Fusion(nn.Module):
    def __init__(self, c1: int, c2: int, n: int = 1, dim: int = 1):
        super(eSE_Fusion, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Conv2d(c1, c2, kernel_size=1,
                             padding=0)
        self.hsigmoid = Hsigmoid()
    ...
    def forward(self, x):
        ...
        Lw = w.size(self.dim)
        assert self.n >= 1
        assert Lx % self.n == 0, f"x.size(dim={self.dim})={Lx} not divisible by n={self.n}"
        assert Lw % self.n == 0, f"w.size(dim={self.dim})={Lw} not divisible by n={self.n}"

        xs = torch.chunk(x, self.n, dim=self.dim)   # tuple length n
        ws = torch.chunk(w, self.n, dim=self.dim)   # tuple length n
...
class Add(nn.Module):
    """Element-wise add for 2 or more tensors with same shape."""
    def __init__(self):
        super().__init__()
...
class ChunkSelect(nn.Module):
    def __init__(self, n=2, i=0, dim=1):
        super().__init__()
        self.n = int(n)
        self.i = int(i)
        self.dim = int(dim)
    ...
    def forward(self, x):
        if self.dim == 1:
            assert x.size(1) % self.n == 0, \
                f"Channel {x.size(1)} not divisible by n={self.n}"
        return torch.chunk(x, self.n, dim=self.dim)[self.i]
